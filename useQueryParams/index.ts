...
export const useQueryParams = () => {
   ...
   */
  const getQueryParamAsNumber = (key: string, defaultValue: number = 0): number => {
   ...
   */
  const getQueryParamAsBoolean = (key: string, defaultValue: boolean = false): boolean => {
    const value = searchParams.get(key);
    if (value === null) return defaultValue;
   ...
   */
  const getQueryParamAll = (key: string): string[] => {
